...
def run_step3(
    ...
    # Data directory from Step 1 (class metadata + test data) — must already exist
    data_directory = get_input_directory(params, data_root)
    classhkl_sentinel = data_directory / "MOD_grain_classhkl_angbin.npz"
    if not classhkl_sentinel.exists():
        raise FileNotFoundError(
            f"Step 1 output not found at: {data_directory}\n"
            f"Please run Step 1 successfully before running Step 3.\n"
            f"Expected file: MOD_grain_classhkl_angbin.npz"
        )

    # Model directory from Step 2 — must already exist
    model_directory = get_input_directory(params, model_root if model_root is not None else data_root)
    model_name = f"model_{material_}_{material1_}" if material_ != material1_ else f"model_{material_}"
    model_sentinel = model_directory / f"{model_name}.json"
    if not model_sentinel.exists():
        raise FileNotFoundError(
            f"Step 2 model not found at: {model_directory}\n"
            f"Please run Step 2 successfully before running Step 3.\n"
            f"Expected file: {model_name}.json"
        )
    ...
    # Load test data (generated by Step 1, lives in data_directory)
    print(f"\nLoading test data (batch_size={test_batch_size}, num_batches={num_test_batches})...")
    test_data_path = str(data_directory / "testing_data")
